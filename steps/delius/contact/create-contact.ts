...
export const createContact = async (page: Page, crn: string) => {
    ...
    await Promise.all([selectOption(page, "#addContactForm\\:ContactType", "Initial Appointment (NS)"), waitForAjax(page)])
    ...
    await Promise.all([selectOption(page, "#addContactForm\\:TransferToTeam", "Unallocated Team(N03)"), waitForAjax(page)])

...
export const createContact = async (page: Page, options: ContactOptions) => {
    ...
    await Promise.all([
        selectOption(page, '#addContactForm\\:ContactType', options.type),
        waitForAjax(page),
    ])
    ...
    await Promise.all([
        selectOption(page, '#addContactForm\\:TransferToTeam', options.team),
        waitForAjax(page),
    ])
...
export const createInitialAppointment = async (page: Page, crn: string, eventNumber: string) =>  createContact(page, {
    crn: crn,

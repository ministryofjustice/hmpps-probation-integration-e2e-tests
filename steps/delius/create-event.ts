...
export async function createSentenceEventForCRN(page: Page, crn:string){
    ...
    await page.selectOption('id=addEventForm:Area', {label: 'NPS Wales'})
    await page.selectOption('id=addEventForm:Team', {label: 'NPS - Wrexham - Team 1'})
    await page.selectOption('id=AppearanceType', {label: 'Sentence'})
    await page.selectOption('id=Plea', {label: 'Guilty'})
    await page.selectOption('id=addEventForm:Outcome', {label: 'ORA Community Order'})

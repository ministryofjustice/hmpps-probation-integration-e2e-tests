...
async function addSentenceInformation(page: Page) {
    ...
    await expect(page).toHaveTitle(
        "Provide a brief summary of the person's index offence(s) and offending history - CAS3"
    )
    await page
    ...
    await page.getByRole('checkbox', { name: 'Standard Recall', exact: true }).check()
...
async function enterAccommodationReferralDetails(page: Page) {
    ...
    await expect(page).toHaveTitle(
        'Has the Duty to refer (England) or Application for Assistance (Wales) been submitted? - CAS3'
    )
    await page.getByRole('radio', { name: 'No', exact: true }).check()
    await page.getByRole('button', { name: 'Save and continue' }).click()
    await expect(page).toHaveTitle(
        'Has a referral to Commissioned Rehabilitative Services (CRS) been submitted? - CAS3'
    )
    await page.getByRole('radio', { name: 'Yes', exact: true }).check()
    await page.getByRole('button', { name: 'Save and continue' }).click()

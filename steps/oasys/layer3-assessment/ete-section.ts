...
export const completeETESection = async (page: Page) => {
    ...
    await page.getByLabel('Any educational or formal professional / vocational qualifications (optional)').selectOption({label: '2 - No qualifications'})
    await page.getByLabel('Attitude to education / training (optional)').selectOption({label: '2-Significant problems'})
    ...
    await page.getByLabel('Education / training / employability issues linked to risk of serious harm, risks to the individual & other risks').selectOption({label: 'Yes'})
    await page.getByLabel('Education / training / employability issues linked to offending behaviour').selectOption({label: 'Yes'})

...
import {navigateToNSIContactDetails, rescheduleSupplierAssessmentAppointment, verifyNSIContactInDelius,
} from '../../steps/delius/contact/find-contacts.js'
import {addDays, addMinutes} from 'date-fns'
import {faker} from "@faker-js/faker";
import {Tomorrow} from "../../steps/delius/utils/date-time.js";
import {createAndAssignReferral} from "./create-referral.spec.js";
test.beforeEach(async ({ page }) => {
    await loginDelius(page)
})

test('Update Supplier Assessment Appointment in Refer and Monitor to future date and Verify Changes in Delius', async ({ page }) => {
    test.slow()
    ...
    const initialAppointmentDateTime = await createSupplierAssessmentAppointment(page, referralRef, addDays(new Date(), 2));
    ...
    await verifyNSIContactInDelius(page, '1 - CRS Accommodation', initialAppointmentDateTime, 'Appointment with CRS Staff (NS)', '');
    ...
    const rescheduledAppointmentDateTime = await rescheduleSupplierAssessmentAppointment(page, referralRef, newAppointmentDate);
    ...
    await loginDelius(page);
    await navigateToNSIContactDetails(page, crn);
    await verifyNSIContactInDelius(page, '1 - CRS Accommodation', rescheduledAppointmentDateTime, 'Appointment with CRS Staff (NS)', '');
    await verifyNSIContactInDelius(page, '1 - CRS Accommodation', initialAppointmentDateTime, 'Appointment with CRS Staff (NS)', 'Rescheduled - Service Request', 'N', 'Y');
});

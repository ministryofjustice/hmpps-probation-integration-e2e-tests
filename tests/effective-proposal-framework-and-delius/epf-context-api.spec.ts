...
import { createCommunityEvent, createCustodialEvent} from '../../steps/delius/event/create-event.js'
...
test('test epf context api endpoint', async ({ page }) => {
    ...
    const crn: string = await createOffender(page, { person: person, providerName: data.teams.referAndMonitorTestTeam.providerName })
    const event = await createCommunityEvent(page, { crn, allocation: data.teams.referAndMonitorTestTeam })
    ...
    expect(json.responsibleProvider.name).toBe(data.teams.referAndMonitorTestTeam.providerName);
